...
public class SendEmbed {
    public static void send_error_message(String command, String error, CommandEvent event){
        event.reply(new EmbedBuilder().setTitle("oops").setDescription("issue with "+command+":\n"+error).setColor(Color.BLUE).build());
    }
